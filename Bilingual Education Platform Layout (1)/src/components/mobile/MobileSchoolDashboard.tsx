...
import { MobileAnnouncementsAdminScreen } from "./screens/MobileAnnouncementsAdminScreen";
import { MobileAnnouncementsParentScreen } from "./screens/MobileAnnouncementsParentScreen";
import { MobileDailyActivitiesAdminScreen } from "./screens/MobileDailyActivitiesAdminScreen";
import { MobileDailyActivitiesParentScreen } from "./screens/MobileDailyActivitiesParentScreen";
...
import { MobileTeachersAdminScreen } from "./screens/MobileTeachersAdminScreen";
import { MobileTeachersParentScreen } from "./screens/MobileTeachersParentScreen";
import { MobileClassesAdminScreen } from "./screens/MobileClassesAdminScreen";
import {
  Calendar,
  Image,
  Users,
...
export function MobileSchoolDashboard() {
    ...
    switch (currentScreen) {
      ...
      case "announcements":
        return userRole === "admin" ? (
          <MobileAnnouncementsAdminScreen />
        ) : (
          <MobileAnnouncementsParentScreen />
        );
      case "messages":
        return <MobileMessagesScreen userRole={userRole} />;
      case "attendance":
      ...
      case "activities":
        return userRole === "admin" ? (
          <MobileDailyActivitiesAdminScreen />
        ) : (
          <MobileDailyActivitiesParentScreen />
        );
      case "albums":
        return (
      ...
      case "classes":
        return userRole === "admin" ? (
          <MobileClassesAdminScreen />
        ) : (
          <MobileComingSoonScreen
            title="Classes"
            description="View your child's class information. Coming soon!"
            icon={Users}
          />
        );
      case "teachers":
        return userRole === "admin" ? (
          <MobileTeachersAdminScreen />
        ) : (
          <MobileTeachersParentScreen />
        );
      case "progress":
        return (
          <MobileComingSoonScreen
  ...
  return (
    <MobileDashboardLayout
      ...
      userRole={userRole}
      onRoleToggle={() => setUserRole(userRole === "admin" ? "parent" : "admin")}
    >
      {renderScreen()}
    </MobileDashboardLayout>
  );
