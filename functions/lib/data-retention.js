...
exports.getRetentionPolicy = exports.processAccountDeletion = exports.exportUserData = exports.cancelAccountDeletion = exports.requestAccountDeletion = void 0;
...
exports.requestAccountDeletion = (0, https_1.onCall)(async (request) => {
    try {
        ...
        if (!confirmDeletion) {
            throw new https_1.HttpsError('invalid-argument', 'Deletion confirmation required');
        }
        ...
        if (!existingRequest.empty) {
            throw new https_1.HttpsError('failed-precondition', 'Deletion request already pending');
        }
        ...
        firebase_functions_1.logger.info(`Account deletion requested for user ${auth.uid}`);
        return {
            success: true,
        ...
        firebase_functions_1.logger.error('Error requesting account deletion:', error);
        throw error;
    }
});
/**
...
exports.cancelAccountDeletion = (0, https_1.onCall)(async (request) => {
    try {
        ...
        firebase_functions_1.logger.info(`Account deletion cancelled for user ${auth.uid}`);
        return {
            success: true,
        ...
        firebase_functions_1.logger.error('Error cancelling account deletion:', error);
        throw error;
    }
});
/**
...
exports.exportUserData = (0, https_1.onCall)(async (request) => {
    try {
        ...
        const existingRequest = await db.collection('export_requests')
            ...
            .get();
        if (!existingRequest.empty) {
            throw new https_1.HttpsError('failed-precondition', 'Export request already pending');
        }
        ...
        firebase_functions_1.logger.info(`Data export requested for user ${auth.uid}`);
        return {
            success: true,
        ...
        firebase_functions_1.logger.error('Error requesting data export:', error);
        throw error;
    }
});
/**
 ...
 */
async function processExportRequest(exportId, userId) {
    try {
        ...
        firebase_functions_1.logger.info(`Data export completed for user ${userId}`);
    }
    catch (error) {
        firebase_functions_1.logger.error('Error processing export request:', error);
        // Update export request status
...
async function collectUserData(userId) {
    ...
    try {
        ...
        userData.bookings = bookingsResponse.records.map((record) => record.fields);
        // Get user posts
        ...
        userData.posts = postsResponse.records.map((record) => record.fields);
        // Get user comments
        ...
        userData.comments = commentsResponse.records.map((record) => record.fields);
        // Get user reviews
        ...
        userData.reviews = reviewsResponse.records.map((record) => record.fields);
        ...
        firebase_functions_1.logger.error('Error collecting user data:', error);
        throw error;
    }
    return userData;
}
...
exports.processAccountDeletion = (0, https_1.onCall)(async (request) => {
    try {
        ...
        firebase_functions_1.logger.info(`Account deleted for user ${userId} by admin ${auth.uid}`);
        return {
            success: true,
        ...
        firebase_functions_1.logger.error('Error processing account deletion:', error);
        throw error;
    }
});
/**
...
async function deleteUserData(userId) {
    try {
        ...
        firebase_functions_1.logger.info(`User data deleted for ${userId}`);
    }
    catch (error) {
        firebase_functions_1.logger.error('Error deleting user data:', error);
        throw error;
    }
}
/**
 ...
 */
async function deleteUserDataFromAirtable(userId) {
    try {
        // Delete user record
        const userResponse = await callAirtableApi('Users', 'GET', {
            params: {
        ...
        firebase_functions_1.logger.error('Error deleting user data from Airtable:', error);
        throw error;
    }
}
/**
...
async function deleteUserDataFromFirestore(userId) {
    try {
        ...
        firebase_functions_1.logger.error('Error deleting user data from Firestore:', error);
        throw error;
    }
}
/**
 ...
 */
async function sendExportNotification(userId, exportId) {
    try {
        // In a real implementation, you would send an email here
        // For now, we'll just log the notification
        firebase_functions_1.logger.info(`Export notification for user ${userId}: ${exportId}`);
        ...
        firebase_functions_1.logger.error('Error sending export notification:', error);
 ...
 */
exports.getRetentionPolicy = (0, https_1.onCall)(async (request) => {
    try {
        const { auth } = request;
        if (!auth) {
            throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
        ...
        firebase_functions_1.logger.error('Error getting retention policy:', error);
        throw error;
    }
});
