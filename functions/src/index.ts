...
async function readSecretOrConfig(name: 'AIRTABLE_PAT' | 'AIRTABLE_BASE'): Promise<string | undefined> {
  try {
    // Try env-first in v1
...
const airtableRequest = async <T>(
  ...
  const resp = await axios.request<T>({ url: fullUrl, method, headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, data: body, validateStatus: () => true });
...
app.post('/ensure-schema', async (req: Request, res: Response) => {
  try {
    ...
    if (!table || !Array.isArray(fields)) return res.status(400).json({ message: 'Invalid payload' });
    // Fetch current table schema
    const result = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/meta/bases/${base}/tables`, undefined, undefined, pat, ''),
    );
    const tbl = (result?.tables || []).find((t: any) => t.name === table);
    if (!tbl) return res.status(404).json({ message: `Table ${table} not found` });
    const existing: Record<string, any> = {};
    tbl.fields.forEach((f: any) => (existing[f.name] = f));
    const toCreate = fields.filter((f: any) => !existing[f.name]);
    if (toCreate.length === 0) return res.json({ created: 0 });
    // Add fields individually
    await withSecrets(async (pat, base) => {
      for (const f of toCreate) {
        await airtableRequest<any>(
          'post',
          `/meta/bases/${base}/tables/${tbl.id}/fields`,
          { fields: [f] },
          undefined,
          pat,
          '',
...
const withSecrets = async <T>(handler: (pat: string, base: string) => Promise<T>): Promise<T> => {
  const [patVal, baseVal] = await Promise.all([
    readSecretOrConfig('AIRTABLE_PAT'),
    readSecretOrConfig('AIRTABLE_BASE'),
  ]);
  if (!patVal || !baseVal) throw new Error('Airtable credentials not configured');
  return handler(patVal, baseVal);
};

// --- Public Providers Endpoints (no auth) ---
app.post('/api/providers/search', async (req: Request, res: Response) => {
  try {
    ...
    if (q) filters.push(`FIND(LOWER('${String(q).toLowerCase()}'), LOWER({displayName}))`);
    if (Array.isArray(subjects) && subjects.length) {
      filters.push(`OR(${subjects.map((s: string) => `FIND('${s}', ARRAYJOIN({subjects}))`).join(',')})`);
    }
    if (priceMin || priceMax) {
      if (priceMin) filters.push(`{priceMin} >= ${Number(priceMin)}`);
      if (priceMax) filters.push(`{priceMax} <= ${Number(priceMax)}`);
    }
    if (modalities && Array.isArray(modalities) && modalities.length) {
      filters.push(`OR(${modalities.map((m: string) => `{modalities.${m}} = 1`).join(',')})`);
    }
    const filterByFormula = filters.length ? `AND(${filters.join(',')})` : '';
    const data = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/Providers`, undefined, filterByFormula ? { filterByFormula, pageSize: 50 } : { pageSize: 50 }, pat, base),
    );
...
app.post('/api/providers/get', async (req: Request, res: Response) => {
  try {
    const providerId = String(req.body?.providerId || '').trim();
    if (!providerId) return res.status(400).json({ message: 'providerId required' });
    const rec = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/Providers/${encodeURIComponent(providerId)}`, undefined, undefined, pat, base),
    );
...
// CRUD routes
app.get('/tables/:table', async (req: Request, res: Response) => {
  try {
    const { table } = req.params;
    const { filterByFormula, maxRecords, pageSize, sort, offset } = req.query as Record<string, string>;
    ...
    const data = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/${encodeURIComponent(table)}`, undefined, query, pat, base),
    );
...
app.get('/tables/:table/:id', async (req: Request, res: Response) => {
  try {
    const { table, id } = req.params;
    const data = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/${encodeURIComponent(table)}/${encodeURIComponent(id)}`, undefined, undefined, pat, base),
    );
...
app.post('/tables/:table', async (req: Request, res: Response) => {
  try {
    const { table } = req.params;
    const { fields } = req.body || {};
    const data = await withSecrets(async (pat, base) =>
      airtableRequest<any>('post', `/${encodeURIComponent(table)}`, { fields }, undefined, pat, base),
    );
    res.json(normalizeRecord(data));
  } catch (e) {
    const err = e as AxiosError | Error;
...
app.patch('/tables/:table/:id', async (req: Request, res: Response) => {
  try {
    const { table, id } = req.params;
    const { fields } = req.body || {};
    const data = await withSecrets(async (pat, base) =>
...
app.delete('/tables/:table/:id', async (req: Request, res: Response) => {
  try {
    const { table, id } = req.params;
    await withSecrets(async (pat, base) =>
      airtableRequest<any>('delete', `/${encodeURIComponent(table)}/${encodeURIComponent(id)}`, undefined, undefined, pat, base),
    );
...
app.post('/likes', async (req: Request, res: Response) => {
  try {
    ...
    const existsResp = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/TutoPostLikes`, undefined, { filterByFormula: `AND({Post ID} = '${postId}', {User ID} = '${userId}')`, maxRecords: 1 }, pat, base),
    );
    if ((existsResp.records || []).length > 0) return res.json(normalizeRecord(existsResp.records[0]));
    // Create like
    const created = await withSecrets(async (pat, base) =>
      airtableRequest<any>('post', `/TutoPostLikes`, { fields: { 'Post ID': postId, 'User ID': userId, 'Created At': new Date().toISOString() } }, undefined, pat, base),
    );
...
app.delete('/likes', async (req: Request, res: Response) => {
  try {
    const { postId, userId } = (req.body || req.query) as any;
    if (!postId || !userId) return res.status(400).json({ message: 'postId and userId are required' });
    // Find like record
    const existsResp = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/TutoPostLikes`, undefined, { filterByFormula: `AND({Post ID} = '${postId}', {User ID} = '${userId}')`, maxRecords: 1 }, pat, base),
    ...
    // Delete like
    await withSecrets(async (pat, base) =>
      airtableRequest<any>('delete', `/TutoPostLikes/${rec.id}`, undefined, undefined, pat, base),
    );
    res.json({ success: true });
  } catch (e) {
    const err = e as AxiosError | Error;
...
app.get('/likes/count', async (req: Request, res: Response) => {
  try {
    const postId = String((req.query?.postId || '')).trim();
    if (!postId) return res.status(400).json({ message: 'postId is required' });
    const resp = await withSecrets(async (pat, base) =>
      airtableRequest<any>(
        'get',
        `/TutoPostLikes`,
        undefined,
        { filterByFormula: `{Post ID} = '${postId}'`, pageSize: 1000, maxRecords: 1000 },
        pat,
        base,
      ),
    );
...
const verifyIdToken = async (req: Request, res: Response, next: any) => {
	try {
...
app.post('/api/users/getByUid', async (req: Request, res: Response) => {
	try {
		const uid = (req.body?.uid as string) || (req as any).user?.uid;
		if (!uid) return res.status(400).json({ ok: false, code: 'MISSING_UID' });
		const data = await withSecrets(async (pat, base) =>
			airtableRequest<any>(
				'get',
				`/Users`,
				undefined,
				{ filterByFormula: `{firebaseUid} = '${uid}'`, maxRecords: 1 },
				pat,
				base,
			),
		);
...
app.post('/api/users/upsertRole', async (req: Request, res: Response) => {
	try {
		...
		const found = await withSecrets(async (pat, base) =>
			airtableRequest<any>(
				'get',
				`/Users`,
				undefined,
...
const resolveGuardianUserId = async (uidFromToken: string): Promise<string | null> => {
	const found = await withSecrets(async (pat, base) =>
		airtableRequest<any>(
			'get',
			`/Users`,
			undefined,
			{ filterByFormula: `{firebaseUid} = '${uidFromToken}'`, maxRecords: 1 },
			pat,
			base,
		),
	);
...
const failedAttempts: Record<string, { count: number; firstAt: number; blockedUntil?: number }> = {};
...
const checkRateLimit = (key: string) => {
	...
	if (item?.blockedUntil && now < item.blockedUntil) return false;
	if (item && now - item.firstAt > RATE_LIMIT_WINDOW_MS) delete failedAttempts[key];
	return true;
};
...
const clearFails = (key: string) => { delete failedAttempts[key]; };

// Ensure all guardian endpoints are behind /api and token middleware
// POST /api/guardian/lookupCode { code }
app.post('/api/guardian/lookupCode', async (req: Request, res: Response) => {
	try {
		const ipKey = `${req.ip}:lookupCode`;
		if (!checkRateLimit(ipKey)) return res.status(429).json({ ok: false, code: 'TOO_MANY_ATTEMPTS', message: 'Too many attempts. Try later.' });
		const rawCode = String((req.body?.code || '')).trim();
		if (!rawCode) return res.status(400).json({ ok: false, code: 'INVALID_CODE', message: 'Code required' });
		const code = rawCode.toUpperCase();
		const invite = await withSecrets(async (pat, base) =>
			airtableRequest<any>('get', `/InviteCodes`, undefined, { filterByFormula: `{code} = '${code}'`, maxRecords: 1 }, pat, base),
		);
		...
		const expiresAt = fields.expiresAt as string | undefined;
		...
		const studentId = fields.studentId as string;
		if (!studentId) {
			registerFail(ipKey);
			return res.status(404).json({ ok: false, code: 'INVALID_CODE', message: 'Invalid code' });
		}
		const student = await withSecrets(async (pat, base) =>
			airtableRequest<any>('get', `/Students/${encodeURIComponent(studentId)}`, undefined, undefined, pat, base),
		);
		const sFields = student.fields || {};
		const fullName = (sFields.fullName as string) || '';
		const grade = (sFields.grade as string) || '';
		clearFails(ipKey);
...
app.post('/api/guardian/createLink', async (req: Request, res: Response) => {
	try {
		const uid = (req as any).user?.uid as string;
		const guardianUserId = await resolveGuardianUserId(uid);
		if (!guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED', message: 'User not found' });
		const studentId = String(req.body?.studentId || '').trim();
		const method = String(req.body?.method || '').trim();
		if (!studentId || !['code', 'qr', 'id'].includes(method)) return res.status(400).json({ ok: false, code: 'INVALID_REQUEST', message: 'Invalid params' });
		// Already linked?
		const links = await withSecrets(async (pat, base) =>
			airtableRequest<any>('get', `/GuardianStudentLinks`, undefined, { filterByFormula: `AND({guardianUserId} = '${guardianUserId}', {studentId} = '${studentId}')`, maxRecords: 1 }, pat, base),
		);
		...
		// Create pending link
		const created = await withSecrets(async (pat, base) =>
			airtableRequest<any>('post', `/GuardianStudentLinks`, { fields: {
				guardianUserId,
				studentId,
				linkMethod: method,
				status: 'pending',
...
app.post('/api/guardian/getLinkById', async (req: Request, res: Response) => {
	try {
		const uid = (req as any).user?.uid as string;
		const guardianUserId = await resolveGuardianUserId(uid);
		if (!guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED', message: 'User not found' });
		const linkId = String(req.body?.linkId || '').trim();
		if (!linkId) return res.status(400).json({ ok: false, code: 'INVALID_REQUEST', message: 'linkId required' });
		const link = await withSecrets(async (pat, base) =>
			airtableRequest<any>('get', `/GuardianStudentLinks/${encodeURIComponent(linkId)}`, undefined, undefined, pat, base),
		);
		if (!link || link.fields?.guardianUserId !== guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED', message: 'Not your link' });
...
app.post('/api/guardian/getLinksForGuardian', async (req: Request, res: Response) => {
	try {
		const uid = (req as any).user?.uid as string;
		const guardianUserId = await resolveGuardianUserId(uid);
		if (!guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED', message: 'User not found' });
		const links = await withSecrets(async (pat, base) =>
			airtableRequest<any>('get', `/GuardianStudentLinks`, undefined, { filterByFormula: `{guardianUserId} = '${guardianUserId}'` }, pat, base),
		);
		const out = (links?.records || []).map((r: any) => ({ id: r.id, studentId: r.fields?.studentId, status: r.fields?.status }));
...
app.post('/api/guardian/revokeLink', async (req: Request, res: Response) => {
	try {
		const uid = (req as any).user?.uid as string;
		const guardianUserId = await resolveGuardianUserId(uid);
		if (!guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED', message: 'User not found' });
		...
		if (!link || link.fields?.guardianUserId !== guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED', message: 'Not your link' });
		await withSecrets(async (pat, base) =>
			airtableRequest<any>('patch', `/GuardianStudentLinks/${link.id}`, { fields: { status: 'revoked', revokedReason: reason || undefined } }, undefined, pat, base),
		);
		return res.json({ ok: true });
	} catch (e) {
		const err = e as Error;
...
app.post('/api/students/searchById', async (req: Request, res: Response) => {
  try {
    const ipKey = `${req.ip}:searchById`;
    if (!checkRateLimit(ipKey)) return res.status(429).json({ ok: false, code: 'TOO_MANY_ATTEMPTS', message: 'Too many attempts. Try later.' });
    const raw = String(req.body?.studentCode || '').trim();
    if (!raw) return res.status(400).json({ ok: false, code: 'INVALID_REQUEST', message: 'studentCode required' });
    const studentCode = raw.toUpperCase();
    const result = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/Students`, undefined, { filterByFormula: `{studentCode} = '${studentCode}'`, maxRecords: 1 }, pat, base),
    );
    ...
    const fields = rec.fields || {};
    const fullName = (fields.fullName as string) || '';
    const grade = (fields.grade as string) || '';
    clearFails(ipKey);
...
app.post('/api/students/getByQrToken', async (req: Request, res: Response) => {
  try {
    const ipKey = `${req.ip}:getByQrToken`;
    if (!checkRateLimit(ipKey)) return res.status(429).json({ ok: false, code: 'TOO_MANY_ATTEMPTS', message: 'Too many attempts. Try later.' });
    const qrToken = String(req.body?.qrToken || '').trim();
    if (!qrToken) return res.status(400).json({ ok: false, code: 'INVALID_REQUEST', message: 'qrToken required' });
    const result = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/Students`, undefined, { filterByFormula: `{qrToken} = '${qrToken}'`, maxRecords: 1 }, pat, base),
    );
    const rec = (result?.records || [])[0];
    if (!rec) {
      registerFail(ipKey);
...
app.post('/api/studentProfiles/create', async (req: Request, res: Response) => {
  try {
    const uid = (req as any).user?.uid as string;
    if (!uid) return res.status(401).json({ ok: false, code: 'NO_TOKEN' });
    const guardianUserId = await resolveGuardianUserId(uid);
    if (!guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED' });
    const { fullName, grade, yearOfBirth, notes } = req.body || {};
    if (!fullName || !grade) return res.status(400).json({ ok: false, code: 'INVALID_REQUEST' });
    const created = await withSecrets(async (pat, base) =>
      airtableRequest<any>('post', `/StudentProfiles`, { fields: { fullName, grade, yearOfBirth, notes, guardianUserId, createdAt: new Date().toISOString() } }, undefined, pat, base),
    );
...
app.post('/api/studentProfiles/listForGuardian', async (req: Request, res: Response) => {
  try {
    const uid = (req as any).user?.uid as string;
    if (!uid) return res.status(401).json({ ok: false, code: 'NO_TOKEN' });
    const guardianUserId = await resolveGuardianUserId(uid);
    if (!guardianUserId) return res.status(403).json({ ok: false, code: 'NOT_AUTHORIZED' });
    const data = await withSecrets(async (pat, base) =>
      airtableRequest<any>('get', `/StudentProfiles`, undefined, { filterByFormula: `{guardianUserId} = '${guardianUserId}'` }, pat, base),
    );
    const items = (data?.records || []).map((r: any) => ({ id: r.id, ...(r.fields || {}) }));
