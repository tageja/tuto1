...
import { getBlockedUsers, unblockUser } from '../../services/social/moderation.service';
...
export default function BlockedUsersScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const list = await getBlockedUsers(profile.id);
      setUsers(list);
    } catch (err) {
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <MaterialIcons name="block" size={48} color="#D1D5DB" />
            <Text style={styles.emptyText}>{t('community.blocked.empty')}</Text>
          </View>
        }
        refreshControl={
        ...
        renderItem={({ item }) => (
          <View style={styles.row}>
            ...
            >
              <Text style={styles.unblockText}>{t('community.blocked.unblock')}</Text>
            </Pressable>
          </View>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  unblockText: { fontSize: 14, color: '#6B7280', fontWeight: '500' },
});
