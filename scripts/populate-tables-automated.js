...
console.log('🚀 Automated Airtable Data Population for TutoApp');
console.log('================================================\n');
...
// Helper function for Metadata API calls (to get table IDs)
...
// Get table IDs from Metadata API
async function getTableIds() {
  try {
    ...
    console.error('❌ Failed to get table IDs:', error.message);
    throw error;
  }
}
...
async function populateTables() {
  try {
    ...
    for (const [tableName, records] of Object.entries(sampleData)) {
      ...
      console.log(`📝 Adding ${records.length} records to ${tableName} (${tableId})...`);
      ...
      try {
        ...
        for (let i = 0; i < airtableRecords.length; i += batchSize) {
          ...
          try {
            const result = await batchCreateRecords(tableId, batch);
        ...
        console.log(`   🎉 Successfully populated ${tableName} with ${totalCreated} records\n`);
