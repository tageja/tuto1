...
console.log('🚀 Delete and Recreate School Tables with Complete Fields');
console.log('==========================================================\n');
...
// School table names to delete and recreate
const schoolTableNames = [
  'TutoSchools',
...
// School table schemas with complete field definitions
...
async function deleteAndRecreateSchoolTables() {
  try {
    ...
    console.log('⚠️  WARNING: This will DELETE all existing school tables and their data!');
    console.log('   This action cannot be undone.\n');
    
    // Find existing school tables
    const existingTables = baseInfo.tables.filter(table => 
      schoolTableNames.includes(table.name)
    );
    
    if (existingTables.length === 0) {
      console.log('ℹ️  No existing school tables found to delete.');
    } else {
      console.log(`📋 Found ${existingTables.length} existing school tables to delete:`);
    ...
    console.log('🗑️  Deleting existing school tables...\n');
    
    for (const table of existingTables) {
      console.log(`🗑️  Deleting table: ${table.name}`);
      try {
        await callMetadataAPI(`/tables/${table.id}`, 'DELETE');
    ...
    // Recreate tables with complete fields
    console.log('📋 Recreating school tables with complete fields...\n');
    
    const recreatedTables = {};
    ...
    for (const [tableName, schema] of Object.entries(schoolTableSchemas)) {
      ...
      try {
        ...
        if (result && result.id) {
          recreatedTables[tableName] = result.id;
    ...
    console.log('🎯 Table Recreation Summary:');
    console.log('============================');
    console.log('');
    
    if (Object.keys(recreatedTables).length > 0) {
      console.log('✅ Successfully recreated tables:');
      Object.entries(recreatedTables).forEach(([name, id]) => {
        console.log(`   - ${name}: ${id}`);
      });
    } else {
      console.log('⚠️  No tables were recreated');
    }
    
    console.log('');
    console.log('🎉 Next Steps:');
    console.log('1. Populate tables with sample data: npm run populate:school:data:auto');
...
// Run the table deletion and recreation
