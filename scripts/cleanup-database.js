...
console.log('🧹 Airtable Database Cleanup for TutoApp');
console.log('==========================================\n');
...
async function deleteTable(tableId) {
  try {
    await callMetadataAPI(`/tables/${tableId}`, 'DELETE');
    console.log(`🗑️  Deleted table: ${tableId}`);
    return true;
  } catch (error) {
    console.error(`❌ Error deleting table ${tableId}:`, error.message);
    return false;
  }
}
...
const tableSchemas = {
  ...
  'Parents': {
    ...
    fields: [
      ...
      { name: 'Phone', type: 'phoneNumber' },
      { name: 'Address', type: 'multilineText' },
      { name: 'Children', type: 'multipleRecordLinks', options: { linkedTableId: 'tblStudents' } },
      { 
        name: 'Payment Method', 
        type: 'singleSelect', 
        options: {
...
async function cleanupDatabase() {
  try {
    ...
    // Tables to keep (our target schema)
    const targetTableNames = Object.keys(tableSchemas);
    console.log('🎯 Target tables for TutoApp:');
    targetTableNames.forEach(name => {
      console.log(`   - ${name}`);
    });
    console.log('');
    
    // Find tables to delete (existing but not in target)
    const tablesToDelete = existingTables.filter(table => 
      !targetTableNames.includes(table.name)
    );
    
    if (tablesToDelete.length > 0) {
      console.log('🗑️  Tables to delete (not in target schema):');
      ...
      console.log('🗑️  Deleting unwanted tables...');
      ...
      console.log('✅ No unwanted tables found.\n');
    }
    
    // Delete and recreate target tables to ensure correct schema
    console.log('🔄 Recreating target tables with correct schema...');
    const targetTables = existingTables.filter(table => 
      targetTableNames.includes(table.name)
    );
    
    for (const table of targetTables) {
      console.log(`🗑️  Deleting existing table: ${table.name}`);
      await deleteTable(table.id);
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
    ...
    console.log('3. Your database is now clean and ready for production!');
