 ...
 * Usage: node scripts/audit-teachers-schema.js
 */
...
function auditTeachersTable(table) {
  const gaps = [];
...
function generateMarkdownReport(result) {
  ...
  if (result.gaps.length === 0) {
    ...
    if (result.summary.autoFixable > 0) {
      ...
      lines.push("   ```bash");
      lines.push("   node scripts/create-teachers-schema.js");
      lines.push("   ```");
      lines.push("");
    }
    ...
    lines.push("   ```bash");
    lines.push("   node scripts/audit-teachers-schema.js");
    lines.push("   ```");
  }
...
async function main() {
  try {
    ...
    if (result.gaps.length > 0) {
      console.log(`\n⚠️  Action required: Run create-teachers-schema.js to fix gaps`);
      process.exit(1);
    } else {
