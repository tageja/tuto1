...
async function recreateSchoolTables() {
  try {
    ...
    for (const [tableName, schema] of Object.entries(schoolTableSchemas)) {
      ...
      try {
        ...
        if (result && result.id) {
          ...
          console.log(`   ✅ Successfully recreated table: ${tableName} (ID: ${result.id})`);
        } else {
          console.log(`   ⚠️  Table recreation response:`, result);
        }
        
        console.log('');
        
      } catch (error) {
        console.log(`   ❌ Error recreating ${tableName}: ${error.message}`);
    ...
    if (Object.keys(recreatedTables).length > 0) {
      ...
      console.log('⚠️  No new tables were recreated (they may already exist)');
    }
