...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Đang theo dõi · @${username} | tuto.social`,
  };
}
...
export default async function FollowingPage({ params }: Props) {
  ...
  const { data: profileRow } = await supabase
    ...
    .select('id, display_name, following_count')
  ...
  const followingCount = (profileRow.following_count as number) ?? 0;
  ...
  const following: FollowingProfile[] = [];
  ...
  return (
    <main className="max-w-xl mx-auto px-4 py-6">
      ...
      {following.length === 0 && (
        <p className="text-gray-500 py-8 text-center">Chưa theo dõi ai</p>
      )}
    </main>
  );
}
