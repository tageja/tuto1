/**
 * Minor — M2 `quiz` steps: MCQ hygiene (≥2 options, answer id matches option, trims).
 */
...
test.beforeAll(() => requireSupabaseAdmin('bug-026-m2-quiz-options-and-answers'));
...
test.describe('Bug #026 — M2 quiz MCQ structure and answers', {
  tag: [TAG.regression, TAG.module2, TAG.content, TAG.data, bugTag(26)],
}, () => {
      ...
      questions.forEach((q, qi) => {
        ...
        opts.forEach((o, oi) => {
          ...
          if (textRaw !== textRaw.trim())
            violations.push(`${base} Q${qi + 1} opt[${oid}]: trailing/leading whitespace in text`);
          if (!textVi.trim()) violations.push(`${base} Q${qi + 1} opt[${oid}]: missing text_vi`);
          if (textVi !== textVi.trim()) {
            violations.push(`${base} Q${qi + 1} opt[${oid}]: trailing/leading whitespace in text_vi`);
          }
        });
