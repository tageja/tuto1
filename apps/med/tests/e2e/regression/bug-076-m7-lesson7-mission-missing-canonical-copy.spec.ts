/**
 * Bug #076 — M7 L7 (`pair-practice-paediatric-emergency-escalation`) mission step:
...
test.describe('Bug #076 — M7 L7 mission step lacks missionEn/missionVi in DB', {
  tag: [TAG.regression, TAG.module7, TAG.content, TAG.data, bugTag(76)],
}, () => {
  test.beforeAll(() => {
    requireSupabaseAdmin('bug-076-m7-lesson7-mission-missing-canonical-copy');
  });
  ...
  test('Supabase mission step config includes substantial missionEn or missionVi', async () => {
    ...
    const { data: lesson } = await sb
      ...
      .select('id')
      .eq('module_id', mod!.id)
      .eq('slug', 'pair-practice-paediatric-emergency-escalation')
      .single();
