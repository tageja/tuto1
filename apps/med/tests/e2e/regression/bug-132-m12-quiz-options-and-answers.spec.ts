/**
 * Bug #132 — M12 `quiz` MCQ structure (L1–L3; L4 has no quiz — in-progress lesson).
 */
...
import { loadM12Steps, M12_LESSON_SLUG_NO_QUIZ } from '../_shared/m12-family-communication-emergencies-linter';

test.beforeAll(() => requireSupabaseAdmin('bug-132-m12-quiz-options-and-answers'));
...
test.describe('Bug #132 — M12 quiz MCQ structure and answers', {
  tag: [TAG.regression, TAG.module12, TAG.content, TAG.data, bugTag(132)],
}, () => {
  test('each quiz question has options and correct option id match', async () => {
    ...
    for (const row of steps.filter((s) => s.type === 'quiz' && s.lesson_slug !== M12_LESSON_SLUG_NO_QUIZ)) {
