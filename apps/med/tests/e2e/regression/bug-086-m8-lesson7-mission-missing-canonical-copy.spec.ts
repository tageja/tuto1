/**
 * Bug #086 — M8 L7 (`pair-practice-nurse-to-doctor-verbal-report`) mission step:
...
test.describe('Bug #086 — M8 L7 mission step lacks missionEn/missionVi in DB', {
  tag: [TAG.regression, TAG.module8, TAG.content, TAG.data, bugTag(86)],
}, () => {
  test.beforeAll(() => {
    requireSupabaseAdmin('bug-086-m8-lesson7-mission-missing-canonical-copy');
  });
  ...
  test('Supabase mission step config includes substantial missionEn or missionVi', async () => {
    ...
    const { data: lesson } = await sb
      ...
      .select('id')
      .eq('module_id', mod!.id)
      .eq('slug', 'pair-practice-nurse-to-doctor-verbal-report')
      .single();
