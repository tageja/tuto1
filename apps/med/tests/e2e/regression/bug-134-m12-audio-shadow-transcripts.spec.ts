/**
 * Bug #134 — M12 `audio_shadow` transcript + segments (L1 and L4 on current prod curriculum).
 */
...
import { loadM12Steps, M12_LESSON_SLUGS_WITH_AUDIO_SHADOW } from '../_shared/m12-family-communication-emergencies-linter';

test.beforeAll(() => requireSupabaseAdmin('bug-134-m12-audio-shadow-transcripts'));

test.describe('Bug #134 — M12 audio_shadow transcript + segments', {
  tag: [TAG.regression, TAG.module12, TAG.content, TAG.data, bugTag(134)],
}, () => {
  test('every audio_shadow step has transcript + transcriptSegments with vi', async () => {
    ...
    for (const row of steps.filter((s) => s.type === 'audio_shadow')) {
      if (!M12_LESSON_SLUGS_WITH_AUDIO_SHADOW.has(row.lesson_slug)) {
