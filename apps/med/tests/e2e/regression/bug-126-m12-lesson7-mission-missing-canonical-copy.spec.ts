/**
 * Bug #126 — M12 L7 (`pair-practice-managing-information-requests-during-treatment`) mission step:
...
test.describe('Bug #126 — M12 L7 mission step lacks missionEn/missionVi in DB', {
  tag: [TAG.regression, TAG.module12, TAG.content, TAG.data, bugTag(126)],
}, () => {
  test.beforeAll(() => {
    requireSupabaseAdmin('bug-126-m12-lesson7-mission-missing-canonical-copy');
  });
  ...
  test('Supabase mission step config includes substantial missionEn or missionVi', async () => {
    ...
    const { data: lesson } = await sb
      ...
      .select('id')
      .eq('module_id', mod!.id)
      .eq('slug', 'pair-practice-managing-information-requests-during-treatment')
      .single();
