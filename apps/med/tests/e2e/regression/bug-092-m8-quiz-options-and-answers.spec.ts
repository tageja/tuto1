/**
 * Bug #092 — M8 `quiz` MCQ structure (L1–L3; L4 has no quiz — in-progress lesson).
 */
...
import { loadM8Steps, M8_LESSON_SLUG_NO_QUIZ } from '../_shared/m8-documentation-rapid-reporting-linter';

test.beforeAll(() => requireSupabaseAdmin('bug-092-m8-quiz-options-and-answers'));
...
test.describe('Bug #092 — M8 quiz MCQ structure and answers', {
  tag: [TAG.regression, TAG.module8, TAG.content, TAG.data, bugTag(92)],
}, () => {
  test('each quiz question has options and correct option id match', async () => {
    ...
    for (const row of steps.filter((s) => s.type === 'quiz' && s.lesson_slug !== M8_LESSON_SLUG_NO_QUIZ)) {
