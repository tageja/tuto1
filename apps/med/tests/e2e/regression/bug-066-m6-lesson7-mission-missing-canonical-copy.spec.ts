/**
 * Bug #066 — M6 L7 (`pair-practice-managing-family-demanding-answers`) mission step:
...
test.describe('Bug #066 — M6 L7 mission step lacks missionEn/missionVi in DB', {
  tag: [TAG.regression, TAG.module6, TAG.content, TAG.data, bugTag(66)],
}, () => {
  test.beforeAll(() => {
    requireSupabaseAdmin('bug-066-m6-lesson7-mission-missing-canonical-copy');
  });
  ...
  test('Supabase mission step config includes substantial missionEn or missionVi', async () => {
    ...
    const { data: lesson } = await sb
      ...
      .select('id')
      .eq('module_id', mod!.id)
      .eq('slug', 'pair-practice-managing-family-demanding-answers')
      .single();
