/**
 * Bug #124 — M11 `audio_shadow` transcript + segments (L1 and L4 on current prod curriculum).
 */
...
import { loadM11Steps, M11_LESSON_SLUGS_WITH_AUDIO_SHADOW } from '../_shared/m11-trauma-acute-injuries-linter';

test.beforeAll(() => requireSupabaseAdmin('bug-124-m11-audio-shadow-transcripts'));

test.describe('Bug #124 — M11 audio_shadow transcript + segments', {
  tag: [TAG.regression, TAG.module11, TAG.content, TAG.data, bugTag(124)],
}, () => {
  test('every audio_shadow step has transcript + transcriptSegments with vi', async () => {
    ...
    for (const row of steps.filter((s) => s.type === 'audio_shadow')) {
      if (!M11_LESSON_SLUGS_WITH_AUDIO_SHADOW.has(row.lesson_slug)) {
