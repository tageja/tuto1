...
import { EMERGENCY_M7_LESSON_1_PATH } from '../_shared/emergency-m7-l1-flow';
import { TAG, bugTag } from '../_shared/tags';

test.describe('Bug #80 — unauthenticated learner cannot load Module 7 lesson', {
  tag: [TAG.regression, TAG.module7, TAG.auth, bugTag(80)],
}, () => {
  test('deep link redirects to login with return path', async ({ page }) => {
    ...
    await page.goto(EMERGENCY_M7_LESSON_1_PATH, { waitUntil: 'domcontentloaded' });
    ...
    expect(decoded).toContain('calling-a-code-blue');
  });
});
