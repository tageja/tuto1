/**
 * Bug #024 — Emergency Module **Triage Intake** Lesson 7 (`triage-challenge`): the `mission` step row
 * has empty `missionEn` / `missionVi` in `config`, so `MissionStep.tsx` renders `t.exampleMission`
 * fallback instead of real module copy.
 */
...
test.describe('Bug #024 — M2 L7 mission step lacks missionEn/missionVi in DB', {
  tag: [TAG.regression, TAG.module2, TAG.content, TAG.data, bugTag(24)],
}, () => {
  test.beforeAll(() => {
    requireSupabaseAdmin('bug-024-m2-lesson7-mission-missing-canonical-copy');
  });
