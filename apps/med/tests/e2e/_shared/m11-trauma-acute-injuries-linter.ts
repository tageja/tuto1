/**
 * Data-quality helpers for Emergency course → Module 11
...
export async function loadM11Steps(sb: SupabaseClient): Promise<M11StepRow[]> {
  ...
  const { data: mod, error: mErr } = await sb
    ...
    .eq('slug', M11_MODULE_SLUG)
    .single();
  if (mErr || !mod) throw new Error(`module ${M11_MODULE_SLUG}: ${mErr?.message}`);
