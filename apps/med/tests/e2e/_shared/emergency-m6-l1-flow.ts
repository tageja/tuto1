/**
 * Emergency Nursing Communication — Module 6 (Reassuring Under Pressure) — Lesson 1
 * slug: calming-a-panicking-patient-in-ae
 *
...
export const EMERGENCY_M6_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/calming-a-panicking-patient-in-ae';

/** M1–M5 lessons (40) must read `completed` for M6 L1 unlock — Supabase snapshot 2026-05-19. */
const PREREQ_LESSON_IDS_FOR_M6_L1: string[] = [
...
export async function wireEmergencyM6L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M6_L1) {
...
export async function gotoEmergencyM6Lesson1(page: Page) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M6_LESSON_1_PATH, {
...
const M6_L1_PREVIEW_CARD_COUNT = 6;
...
export async function advanceFlashCardVocabPreview(page: Page) {
  ...
  for (let i = 0; i < M6_L1_PREVIEW_CARD_COUNT; i++) {
...
export async function prepM6L1ReachedAudioShadowStep(page: Page) {
...
export async function advanceM6L1AudioShadowThenCompleteVideo(page: Page) {
...
export const M6_L1_CLOZE_ANSWER_PHRASES = [
  'understand',
  'frightened',
  'everything',
  'calm',
...
export async function answerM6Lesson1RecognitionQuizWrongThenSkip(page: Page) {
  await expect(
    page.getByRole('heading', { level: 3 }).filter({ hasText: /Recognition|scenario check|Check Your Understanding|Kiểm tra|reassur|frighten|panick/i }),
...
export async function completeM6Lesson1ClozeWordBank(page: Page) {
  ...
  for (const phrase of M6_L1_CLOZE_ANSWER_PHRASES) {
...
export async function navigateM6Lesson1ToScriptReadSubtitle(page: Page) {
  await prepM6L1ReachedAudioShadowStep(page);
  await advanceM6L1AudioShadowThenCompleteVideo(page);
  await answerM6Lesson1RecognitionQuizWrongThenSkip(page);
  await completeM6Lesson1ClozeWordBank(page);
