/**
 * Emergency Nursing Communication — Module 10 (Emergency Procedures Communication) — Lesson 1
 * slug: explaining-iv-line-insertion
 *
...
export const EMERGENCY_M10_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/explaining-iv-line-insertion';

/** M1–M9 (72 lessons) — unlock M10 L1 (Supabase snapshot 2026-05-19). */
const PREREQ_LESSON_IDS_FOR_M10_L1: string[] = [
...
export async function wireEmergencyM10L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M10_L1) {
...
export async function gotoEmergencyM10Lesson1(page: Page) {
  const mainTitle = page.locator('main h1').first();
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M10_LESSON_1_PATH, {
...
const M10_L1_PREVIEW_CARD_COUNT = 6;
...
export async function advanceM10L1VideoToFlashCards(page: Page) {
...
export async function advanceFlashCardVocabPreviewToAudioShadow(page: Page) {
  ...
  for (let i = 0; i < M10_L1_PREVIEW_CARD_COUNT; i++) {
...
export async function advanceM10L1AudioShadowToScriptRead(page: Page) {
...
export async function navigateM10Lesson1ToScriptRead(page: Page) {
  await advanceM10L1VideoToFlashCards(page);
  await advanceFlashCardVocabPreviewToAudioShadow(page);
  await advanceM10L1AudioShadowToScriptRead(page);
}
