/**
 * Data-quality helpers for Emergency course → Module 9
...
export async function loadM9Steps(sb: SupabaseClient): Promise<M9StepRow[]> {
  ...
  const { data: mod, error: mErr } = await sb
    ...
    .eq('slug', M9_MODULE_SLUG)
    .single();
  if (mErr || !mod) throw new Error(`module ${M9_MODULE_SLUG}: ${mErr?.message}`);
