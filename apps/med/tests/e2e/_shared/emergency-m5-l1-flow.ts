/**
 * Emergency Nursing Communication — Module 5 (Deterioration & Escalation) — Lesson 1
 * slug: vital-signs-in-crisis-what-the-numbers-mean
 *
...
export const EMERGENCY_M5_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/vital-signs-in-crisis-what-the-numbers-mean';

/** M1–M4 lessons (32) must read `completed` for M5 L1 unlock — Supabase snapshot 2026-05-19. */
const PREREQ_LESSON_IDS_FOR_M5_L1: string[] = [
...
export async function wireEmergencyM5L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M5_L1) {
...
export async function gotoEmergencyM5Lesson1(page: Page) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M5_LESSON_1_PATH, {
...
const M5_L1_PREVIEW_CARD_COUNT = 6;
...
export async function advanceFlashCardVocabPreview(page: Page) {
  ...
  for (let i = 0; i < M5_L1_PREVIEW_CARD_COUNT; i++) {
...
export async function prepM5L1ReachedAudioShadowStep(page: Page) {
...
export async function advanceM5L1AudioShadowThenCompleteVideo(page: Page) {
...
export const M5_L1_CLOZE_ANSWER_PHRASES = [
  'report',
  'concern',
  'dropped',
  '115',
...
export async function answerM5Lesson1RecognitionQuizWrongThenSkip(page: Page) {
  await expect(
    page.getByRole('heading', { level: 3 }).filter({ hasText: /Recognition|scenario check|Check Your Understanding|Kiểm tra|vital|SBAR|deteriorat/i }),
...
export async function completeM5Lesson1ClozeWordBank(page: Page) {
  ...
  for (const phrase of M5_L1_CLOZE_ANSWER_PHRASES) {
...
export async function navigateM5Lesson1ToScriptReadSubtitle(page: Page) {
  await prepM5L1ReachedAudioShadowStep(page);
  await advanceM5L1AudioShadowThenCompleteVideo(page);
  await answerM5Lesson1RecognitionQuizWrongThenSkip(page);
  await completeM5Lesson1ClozeWordBank(page);
