/**
 * Emergency Nursing Communication — Module 9 (Simulation & Emergency Review) — Lesson 1
 * slug: team-debrief-after-resuscitation
 *
 * Step order (L1, in-progress): scenario_intro → flash_card → video → script_read →
 * quiz → cloze → matching — **no audio_shadow** (video-first simulation debrief pattern).
 */
...
export const EMERGENCY_M9_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/team-debrief-after-resuscitation';

/** M1–M7 (56 lessons) — same IDs as M8 L1 gate. */
const PREREQ_M1_THROUGH_M7: string[] = [
...
/** Module 8 — all eight lessons for M9 L1 unlock (Supabase snapshot 2026-05-19). */
const PREREQ_M8_ALL: string[] = [
...
const PREREQ_LESSON_IDS_FOR_M9_L1 = [...PREREQ_M1_THROUGH_M7, ...PREREQ_M8_ALL];

export async function wireEmergencyM9L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M9_L1) {
...
export async function gotoEmergencyM9Lesson1(page: Page) {
  const mainTitle = page.locator('main h1').first();
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M9_LESSON_1_PATH, {
...
const M9_L1_PREVIEW_CARD_COUNT = 6;
...
async function completeFlashDeckAndAdvanceToVideo(page: Page) {
  ...
  await progressSave;
  await expect(page.locator('video').first()).toBeVisible({ timeout: 45_000 });
}
...
export async function advanceFlashCardVocabPreview(page: Page) {
  ...
  if (await main.getByText(/thẻ đã thuộc|cards mastered/i).isVisible({ timeout: 2_000 }).catch(() => false)) {
    await completeFlashDeckAndAdvanceToVideo(page);
    return;
  }
  for (let i = 0; i < M9_L1_PREVIEW_CARD_COUNT; i++) {
    if (await page.locator('video').first().isVisible({ timeout: 500 }).catch(() => false)) {
      return;
    }
...
export async function prepM9L1ReachedVideoStep(page: Page) {
...
/** Complete video step (no audio_shadow on L1). */
export async function advanceM9L1VideoToScriptRead(page: Page) {
  await page.evaluate(() => {
...
export async function navigateM9Lesson1ToScriptRead(page: Page) {
  await prepM9L1ReachedVideoStep(page);
  await advanceM9L1VideoToScriptRead(page);
}
