...
export const M5_MODULE_SLUG = 'communicating-patient-deterioration-escalation-protocols';
...
export async function loadM5Steps(sb: SupabaseClient): Promise<M5StepRow[]> {
  ...
  const { data: mod, error: mErr } = await sb
    ...
    .eq('slug', M5_MODULE_SLUG)
    .single();
  if (mErr || !mod) throw new Error(`module ${M5_MODULE_SLUG}: ${mErr?.message}`);
