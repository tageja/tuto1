/**
 * Emergency Nursing Communication — Module 4 (Common Emergency Scenarios) — Lesson 1
 * slug: chest-pain-and-breathing-emergencies
 *
...
export const EMERGENCY_M4_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/chest-pain-and-breathing-emergencies';

/** M1+M2+M3 lessons (24) must read `completed` for M4 L1 unlock — Supabase snapshot 2026-05-19. */
const PREREQ_LESSON_IDS_FOR_M4_L1: string[] = [
...
export async function wireEmergencyM4L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M4_L1) {
...
export async function gotoEmergencyM4Lesson1(page: Page) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M4_LESSON_1_PATH, {
...
const M4_L1_PREVIEW_CARD_COUNT = 6;
...
export async function advanceFlashCardVocabPreview(page: Page) {
  ...
  for (let i = 0; i < M4_L1_PREVIEW_CARD_COUNT; i++) {
...
export async function prepM4L1ReachedAudioShadowStep(page: Page) {
...
export async function advanceM4L1AudioShadowThenCompleteVideo(page: Page) {
...
export const M4_L1_CLOZE_ANSWER_PHRASES = [
  'you hear me',
  'having chest pain',
  'did this start',
  'arm or jaw',
...
export async function completeM4Lesson1ClozeWordBank(page: Page) {
  ...
  for (const phrase of M4_L1_CLOZE_ANSWER_PHRASES) {
...
export async function navigateM4Lesson1ToScriptReadSubtitle(page: Page) {
  await prepM4L1ReachedAudioShadowStep(page);
  await advanceM4L1AudioShadowThenCompleteVideo(page);
  await answerM4Lesson1RecognitionQuizWrongThenSkip(page);
  await completeM4Lesson1ClozeWordBank(page);
