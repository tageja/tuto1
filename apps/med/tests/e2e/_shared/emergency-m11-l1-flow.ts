/**
 * Emergency Nursing Communication — Module 11 (Trauma and Acute Injuries) — Lesson 1
 * slug: road-traffic-accident-victim-in-ae
 *
...
export const EMERGENCY_M11_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/road-traffic-accident-victim-in-ae';

/** M1–M10 (80 lessons) — unlock M11 L1 (Supabase snapshot 2026-05-19). */
const PREREQ_LESSON_IDS_FOR_M11_L1: string[] = [
...
export async function wireEmergencyM11L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M11_L1) {
...
export async function gotoEmergencyM11Lesson1(page: Page) {
  const mainTitle = page.locator('main h1').first();
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M11_LESSON_1_PATH, {
...
const M11_L1_PREVIEW_CARD_COUNT = 6;
...
export async function advanceM11L1VideoToFlashCards(page: Page) {
...
export async function advanceFlashCardVocabPreviewToAudioShadow(page: Page) {
  ...
  for (let i = 0; i < M11_L1_PREVIEW_CARD_COUNT; i++) {
...
export async function advanceM11L1AudioShadowToScriptRead(page: Page) {
...
export async function navigateM11Lesson1ToScriptRead(page: Page) {
  await advanceM11L1VideoToFlashCards(page);
  await advanceFlashCardVocabPreviewToAudioShadow(page);
  await advanceM11L1AudioShadowToScriptRead(page);
}
