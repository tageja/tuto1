/**
 * Data-quality helpers for Emergency course → Module 7
...
export const M7_MODULE_SLUG = 'red-flags-escalation';
...
export async function loadM7Steps(sb: SupabaseClient): Promise<M7StepRow[]> {
  ...
  const { data: mod, error: mErr } = await sb
    ...
    .eq('slug', M7_MODULE_SLUG)
    .single();
  if (mErr || !mod) throw new Error(`module ${M7_MODULE_SLUG}: ${mErr?.message}`);
