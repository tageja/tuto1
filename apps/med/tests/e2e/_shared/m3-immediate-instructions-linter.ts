/**
 * Data-quality helpers for Emergency course → Module 3 (`immediate-instructions-in-emergencies`) steps.
...
export const M3_MODULE_SLUG = 'immediate-instructions-in-emergencies';

export type M3StepRow = {
  id: string;
  lesson_id: string;
  lesson_slug: string;
  type: string;
...
export async function loadM3Steps(sb: SupabaseClient): Promise<M3StepRow[]> {
  ...
  const { data: mod, error: mErr } = await sb
    ...
    .eq('slug', M3_MODULE_SLUG)
    .single();
  if (mErr || !mod) throw new Error(`module ${M3_MODULE_SLUG}: ${mErr?.message}`);
