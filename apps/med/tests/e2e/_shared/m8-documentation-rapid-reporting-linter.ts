/**
 * Data-quality helpers for Emergency course → Module 8
...
export const M8_MODULE_SLUG = 'documentation-and-rapid-reporting';

/** L4 in-progress — no quiz step (exclude from quiz linter). */
export const M8_LESSON_SLUG_NO_QUIZ = 'handing-over-a-deteriorating-patient-mid-treatment';

/** L4 in-progress — no audio_shadow step. */
export const M8_LESSON_SLUG_NO_AUDIO_SHADOW = M8_LESSON_SLUG_NO_QUIZ;

export const MULTI_WORD_NURSE_ROLE = /^(charge\s+nurse|senior\s+nurse|head\s+nurse|ward\s+nurse)\s*:/i;

export type M8StepRow = {
  id: string;
  lesson_id: string;
  lesson_slug: string;
  type: string;
...
export async function loadM8Steps(sb: SupabaseClient): Promise<M8StepRow[]> {
  ...
  const { data: mod, error: mErr } = await sb
    ...
    .eq('slug', M8_MODULE_SLUG)
    .single();
  if (mErr || !mod) throw new Error(`module ${M8_MODULE_SLUG}: ${mErr?.message}`);
...
export function videoUrlFromConfig(cfg: Record<string, unknown> | null | undefined): string {
  if (!cfg) return '';
  return String(cfg.videoUrl ?? cfg.video_url ?? cfg.url ?? '').trim();
}
