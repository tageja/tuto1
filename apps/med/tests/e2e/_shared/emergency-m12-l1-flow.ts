/**
 * Emergency Nursing Communication — Module 12 (Family Communication in Emergencies) — Lesson 1
 * slug: explaining-a-cardiac-arrest-to-the-family
 *
 * Step order (L1): video → flash_card → audio_shadow → script_read → quiz → cloze → matching
 * **No scenario_intro** — video-first procedures pedagogy.
 */

import { expect, type Page } from '@playwright/test';
import { dismissLessonTourIfPresent, stubPairsMembershipInGroup } from './emergency-m1-l1-flow';

export { dismissLessonTourIfPresent, stubPairsMembershipInGroup };

export const EMERGENCY_M12_LESSON_1_PATH =
  '/learn/courses/emergency-nursing-communication/lessons/explaining-a-cardiac-arrest-to-the-family';

/** M1–M11 (88 lessons) — unlock M12 L1 (Supabase snapshot 2026-05-19). */
const PREREQ_LESSON_IDS_FOR_M12_L1: string[] = [
  ...
  'f2b75975-f7fc-47b4-8d07-4cbd2a62ad9c',
  '1f2b09b0-fa88-428e-88d9-7201962b9e08',
  '1275c045-a0b7-40d0-87a4-5296d0938d64',
  '5ce20fb9-3f0e-4ce5-9b23-3cf49a603503',
  '8cb4081b-2ef1-4489-80b7-41120b034b6c',
  'f35d4f0e-aaeb-4f4f-ada1-9c93de2ef449',
  'fcac4316-47a0-4582-9434-cd891e5fe0de',
  'f38ca34c-85c7-4bc9-8682-2beb6e36d768',
  '2add7342-4c52-4137-8774-b718fe3c5c7b',
  '71dbf6fc-32b8-4ad3-82ca-da2c5f960656',
  'd6de30df-e79d-4ad9-88df-eb7494655bdf',
  'd9224a8e-26c0-4045-ac80-77c62471670d',
  'd30151ad-7abd-402e-9388-863b4b691a90',
  '9317b4c6-1012-4331-9594-cc233a0328a7',
  'c9e510a2-e650-4337-9dbc-c09b55dad51a',
  'db9286a5-759f-43e2-9dd4-5ddab2a19340',
  '99937177-07fa-4ec7-8b00-9633f7e5f46d',
  '979741a9-c89b-4dac-afca-703b5845480c',
  '880a481f-8042-449b-b5a0-5c03288dbba0',
  '3961c91e-ca9d-457d-aefb-9454ccf13b92',
  'dcf6cb01-c941-489b-915b-baacc4ac1809',
  '65765e65-ae96-4665-98b0-c28e5b83537b',
  '8e03f4b5-ed88-408b-9152-022c654b7dbe',
  'c7ea0190-400e-4194-a8af-a6b182b798b2',
  '8b96b1af-14f4-4ccd-acea-9b8fef3493fb',
  '8ccd2759-7a8a-49cf-a655-20210919dcda',
  '5f09e1c0-c4bb-43b8-8edb-880f3525439c',
  'c343e6c1-a202-4da7-b549-d35f3f27fd57',
  '26887def-a14f-4412-a562-eeaf247c3486',
  '238a8f5b-13c6-4a28-a03b-43b28cc49b77',
  'e3a1df88-d658-40d3-b21d-1f898360d8fe',
  '803a243b-281e-4154-a450-bbc4561dd8b9',
];

export async function wireEmergencyM12L1LessonGates(page: Page) {
  ...
  await page.route('**/api/progress/course**', async (route) => {
    ...
    for (const lessonId of PREREQ_LESSON_IDS_FOR_M12_L1) {
...
export async function gotoEmergencyM12Lesson1(page: Page) {
  const mainTitle = page.locator('main h1').first();
  for (let attempt = 0; attempt < 8; attempt++) {
    await page.goto(EMERGENCY_M12_LESSON_1_PATH, {
...
const M12_L1_PREVIEW_CARD_COUNT = 6;
...
async function completeFlashDeckAndAdvanceToAudioShadow(page: Page) {
...
export async function advanceM12L1VideoToFlashCards(page: Page) {
  await expect(page.locator('video').first()).toBeVisible({ timeout: 25_000 });
  await page.evaluate(() => {
  ...
  await expect(watchedBtn).toBeVisible({ timeout: 12_000 });
  await watchedBtn.click();
  await page.waitForTimeout(600);
  await waitForFlashCardDeck(page);
}

export async function advanceFlashCardVocabPreviewToAudioShadow(page: Page) {
  await waitForFlashCardDeck(page);
  const main = page.locator('main');
  if (await main.getByText(/thẻ đã thuộc|cards mastered/i).isVisible({ timeout: 2_000 }).catch(() => false)) {
    await completeFlashDeckAndAdvanceToAudioShadow(page);
    return;
  }
  for (let i = 0; i < M12_L1_PREVIEW_CARD_COUNT; i++) {
...
export async function advanceM12L1AudioShadowToScriptRead(page: Page) {
  const listenTab = page.getByRole('tab', { name: /listen|nghe/i }).first();
  await expect(listenTab).toBeVisible({ timeout: 20_000 });
  ...
  for (let tries = 0; tries < 14; tries++) {
    if (
      await page
        .getByText(/read each line aloud|đọc to từng lượt/i)
...
export async function navigateM12Lesson1ToScriptRead(page: Page) {
  await advanceM12L1VideoToFlashCards(page);
  await advanceFlashCardVocabPreviewToAudioShadow(page);
  await advanceM12L1AudioShadowToScriptRead(page);
}
