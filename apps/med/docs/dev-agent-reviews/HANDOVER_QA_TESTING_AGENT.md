...
|---|---|---|
| 2026-05-23 23:45 | QA Agent | **Studio full E2E journey (bugs 271–289).** Specs: `bug-271-complete-intake-draft-creation` through `bug-289-admin-preview-link-pending-course` (`@studio` + `@e2e`; learner specs also `@learner`). Added `e2e: '@e2e'` and `learner: '@learner'` to `tests/e2e/_shared/tags.ts`. Extended `studio-pages.ts` (`fillFullStudioIntakeForm`, `buildMockStudioCourseTwoModulesPayload`, `mockAdminReviewQueue`, `mockAdminCourseReview`, reference-upload locators) and `learner-pages.ts` (Emergency Nursing id/slug constants). **Mocks:** brainstorm/generate NDJSON, studio course GET (2 modules + 2 video cards), validate/submit, admin review-queue + approve. **Learner:** real published course `emergency-nursing-communication` (id `9113d5cb-cedb-4bea-9678-7321020230e8`) for catalogue → lesson → step progression. **No app REAL_BUG fixes.** **`npx playwright test --grep "@e2e" --project=chromium-desktop` → 22 passed** (+2 setup). **`--grep "@studio"` → 111 passed, 3 failed** (+2 setup; bugs 207–289) — failures are **pre-existing** bugs 216, 226, 247 (not 271–289). **Bug #290 (runner):** `cd apps/med && npx playwright test --grep "@studio" --project=chromium-desktop`. |
| 2026-05-23 23:00 | QA Agent | **Studio Phase 7 — Review workflow (bugs 259–270).** Specs: `bug-259-review-panel-visible` through `bug-270-admin-sidebar-pending-badge` (`@studio` + `@review`). Added `review: '@review'` to `tests/e2e/_shared/tags.ts`. Extended `studio-pages.ts` with `buildMockStudioCoursePayload` review fields, `mockStudioCourseValidate()`, `mockStudioCourseSubmit()`, `mockAdminPendingCount()`, review UI locators, API path constants. **Coverage:** `ReviewStatusPanel` draft validate/submit (mocked validate/submit APIs), inline validation issues, submitted/rejected/published banners, `POST /api/studio/courses/[id]/submit` auth (401/403 + learner 403), admin `/admin/courses` Pending Review tab, `POST /api/admin/courses/[id]/review` (learner 403, reject without notes → 400), sidebar pending-count badge (mock `GET /api/admin/courses/pending-count`). **No app REAL_BUG fixes.** **`npx playwright test --grep "@review" --project=chromium-desktop` → 12 passed** (+2 setup). **`--grep "@studio"` → 92 passed** (+2 setup; bugs 207–270). Re-run: `cd apps/med && npx playwright test --grep "@review" --project=chromium-desktop`. |
| 2026-05-23 19:45 | QA Agent | **Studio Phase 7 — Stencil template system (bugs 249–258).** Specs: `bug-249-template-selector-three-options` through `bug-258-template-build-validation` (`@studio` + `@template`). Added `template: '@template'` to `tests/e2e/_shared/tags.ts`. **Coverage:** 3-template `<select>` (names + EN descriptions + value state), legacy `normalizeTemplateId` (`safety_procedures` → `organisational_training`), unit-style `resolveTemplate` determinism + 8×8 across 10 draftIds, `assertAllTemplates()` import-time guard + invalid pool throw, brainstorm prompt/API (mocked NDJSON via in-page `fetch` + `page.route` — **not** `page.request`, which bypasses route handlers), Organisational Training + Student English Step 1→2 wizard flows, bug-255 documents EN-only template copy with VI page chrome. **No app REAL_BUG fixes.** **`npx playwright test --grep "@template" --project=chromium-desktop` → 27 passed** (+2 setup). **`--grep "@studio"` → 82 passed** (+2 setup; bugs 207–258). Re-run: `cd apps/med && npx playwright test --grep "@template" --project=chromium-desktop`. |
| 2026-05-23 14:15 | QA Agent | **Studio Phase 6 — Manual video production queue (bugs 239–248).** Specs: `bug-239-studio-course-page-loads` through `bug-248-studio-course-unauthenticated-redirect` (`@studio` + `@media-queue`). Extended `tests/e2e/_shared/tags.ts` (`mediaQueue`), `studio-pages.ts` (course API mocks, bilingual tab/locators, admin media paths). **Mocks:** `GET /api/studio/courses/[courseId]` for course overview + video cards; PATCH media mocked in bug-241. **Test harness:** bilingual selectors (VI default locale); `gotoStudioCourse()` accepts custom payload; **bug-236** href → `/studio/{courseId}` (matches Phase 6 `GenerationProgress`). Auth: studio/admin PATCH 403 without session; learner blocked. **`npx playwright test --grep "@media-queue" --project=chromium-desktop` → 13 passed** (+2 setup). **`--grep "@studio"` → 55 passed** (+2 setup; bugs 207–248). **No app REAL_BUG fixes.** Re-run: `cd apps/med && npx playwright test --grep "@media-queue" --project=chromium-desktop`. |
| 2026-05-23 01:30 | QA Agent | **Studio Phase 5 — Template-fill generation (bugs 233–238).** Specs: `bug-233-studio-generate-api-auth` through `bug-238-studio-generation-lesson-progress` (`@studio`). Extended `tests/e2e/_shared/studio-pages.ts` with `STUDIO_GENERATE_API`, `MOCK_GENERATE_NDJSON`, `mockStudioGenerate()`, `mockStudioGenerateStreaming()` (delayed NDJSON via `addInitScript` fetch stub), `reachStudioGenerationStep()`, `advanceStudioToGenerationStep()`, generation UI locators. **Mocks:** `POST /api/studio/generate` NDJSON (`module_start`, `lesson_start`, `lesson_done`, `complete` / `error`); brainstorm + chat unchanged. **Updated bug-231** — Step 3 now asserts `GenerationProgress` (no placeholder). Auth: unauthenticated + learner → 403; admin missing `draftId` → 400. **`npx playwright test --grep "@studio" --project=chromium-desktop` → 44 passed** (+2 setup; bugs 207–238). Full `chromium-desktop` → **301 passed, 6 failed, 3 skipped** — failures are pre-existing auth flakes (bug-002, bug-163–170) + **bug-197** dev server `ERR_CONNECTION_RESET` under long run; not studio. **No app REAL_BUG fixes.** Re-run: `cd apps/med && npx playwright test --grep "@studio" --project=chromium-desktop`. |
| 2026-05-22 17:15 | QA Agent | **Studio Phase 4 — Refinement chat (bugs 226–232).** Specs: `bug-226-studio-chat-api-auth` through `bug-232-studio-chat-mobile-layout` (`@studio`). Extended `tests/e2e/_shared/studio-pages.ts` with `STUDIO_CHAT_API`, `buildE2eValidSynopsis()`, `buildMockChatUiMessageSse()`, `mockStudioChat()`, `reachStudioSynopsisStep()`, chat UI locators. **Mocks:** `POST /api/studio/chat` (AI SDK SSE v1) + brainstorm NDJSON; no real Gemini. **Test harness fix:** `MOCK_BRAINSTORM_NDJSON` complete synopsis now schema-valid (3×8 lessons) so `completeSynopsis` enables Refine/Looks-good buttons. Auth: unauthenticated + learner → 403; admin empty `messages` + `draftId: null` → 200/400/500. **`npx playwright test --grep "@studio" --project=chromium-desktop` → 36 passed** (+2 setup; bugs 207–232). Full `chromium-desktop` → **286 passed, 13 failed, 3 skipped** — failures are pre-existing auth/M1 lesson flakes (bug-002, bug-163–170, bug-182, bug-188, M1 happy-path, etc.), not studio. **No app REAL_BUG fixes.** Re-run: `cd apps/med && npx playwright test --grep "@studio" --project=chromium-desktop`. |
| 2026-05-22 24:15 | QA Agent | **Studio Phase 3 — AI brainstorm (bugs 220–225).** Specs: `bug-220-studio-brainstorm-api-auth` through `bug-225-studio-new-wizard-step-flow` (`@studio`). Extended `tests/e2e/_shared/studio-pages.ts` with `MOCK_BRAINSTORM_NDJSON`, `mockStudioBrainstorm()`, `mockStudioNewWizardApis()`, intake/synopsis helpers. **All brainstorm UI tests mock `POST /api/studio/brainstorm`** (no real Gemini). Auth: unauthenticated + learner → 403; admin empty body → 400. **`npx playwright test --grep "@studio" --project=chromium-desktop` → 27 passed** (+2 setup; bugs 207–225). Full `chromium-desktop` regression → **287 passed, 3 failed, 3 skipped** — failures are pre-existing **bug-002** (logout), **bug-022** (M2 audio_shadow a11y), **bug-170** (auth verify invalid token UI), not studio. **No app REAL_BUG fixes.** Re-run: `cd apps/med && npx playwright test --grep "@studio" --project=chromium-desktop`. |
| 2026-05-22 23:45 | QA Agent | **Studio batch 2 — `/studio` + `/studio/new` (bugs 212–219).** Specs: `bug-212-studio-unauthenticated-redirect` through `bug-219-studio-category-suggestion-form` (`@studio`). **Auth:** gate tests use fresh context / `test@test.com` learner session; creator page-load tests use **`admin@test.com` `super_admin`** via `configureStudioAccess()` in `studio-pages.ts` (same pattern as `@admin-pages`, not `AUTH_DISABLED` — `.env.local` has `NEXT_PUBLIC_AUTH_DISABLED=false`). Mocked `GET /api/studio/drafts` + `GET /api/studio/categories` for stable empty state; `POST /api/studio/category-suggestions` mocked in bug-219. **`npx playwright test --grep "@studio" --project=chromium-desktop` → 19 passed** (+2 setup). Full desktop regression after batch: **274 passed, 8 failed, 3 skipped** — failures are pre-existing auth/HCMUTE/lesson flakes (not studio). **No app REAL_BUG fixes.** Re-run studio: `cd apps/med && npx playwright test --grep "@studio" --project=chromium-desktop`. Refresh auth if skips: `npx playwright test --project=setup`. |
| 2026-05-22 22:20 | QA Agent | **Studio batch 1 — `/become-creator` + studio APIs (bugs 207–211).** Specs: `bug-207-become-creator-page-loads` through `bug-211-studio-drafts-api-unauthenticated` (`@studio`). Shared helpers merged into `tests/e2e/_shared/studio-pages.ts`. App: form field `name` attrs + explicit submit button on `app/become-creator/page.tsx`. **`npx playwright test bug-207..211 --project=chromium-desktop` → 8 passed** (setup + 6). **`--grep "@studio"` → 19 passed** (+2 setup; bug-212–219 from studio batch 2). Re-run: `cd apps/med && npx playwright test --grep "@studio" --project=chromium-desktop`. |
| 2026-05-22 18:30 | QA Agent | **Admin pages batch 4 (final — AD1–AD18).** Exploration: `tests/exploration/findings-admin-pages-batch4.md`. Specs: `bug-190`–`bug-201`, `bug-205`–`bug-206` (tag `@admin-pages`; `tests/e2e/_shared/admin-pages.ts`, `tests/e2e/admin.setup.ts`, `ensureTestAdminProfile` + `buildAdminStorageState` in `supabase-admin.ts`). QA admin: `admin@test.com` / `password` (`super_admin` profile seeded when missing). Lesson editor DnD fix verified in source (`insertAt = dragIndex < dragOverIndex ? dragOverIndex - 1 : dragOverIndex`). **No product REAL_BUG fixes this batch.** **`npx playwright test --grep "@admin-pages" --project=chromium-desktop` → 23 passed** (25 with setup). Full regression: **472 passed, 18 failed, 6 skipped** (+44 vs 428 baseline; failures = known auth/mobile flakes, not admin). **Re-run:** `cd apps/med && npx playwright test --grep "@admin-pages" --project=chromium-desktop`. |
| 2026-05-22 12:00 | QA Agent | **Learner pages batch 3 (`/learn` … `/learn/feedback`).** Exploration: `tests/exploration/findings-learner-pages-batch3.md`. Specs: `bug-177`–`bug-189` (tag `@learner-pages`; shared `tests/e2e/_shared/learner-pages.ts`). **REAL_BUG:** `/learn/profile` 404 — missing route (now `app/learn/profile/page.tsx`); guard `PGRST116` in `lib/db/profile.ts` (~line 246). **`npx playwright test --grep "@learner-pages"` → 27 passed** (13 specs × 2 projects + setup). Full regression: **428 passed, 15 failed, 6 skipped** — +22 tests vs prior 406 baseline; failures align with known auth/mobile flakes under long parallel load, not new learner desktop regressions. **Re-run batch:** `cd apps/med && npx playwright test --grep "@learner-pages"`. |
| 2026-05-21 23:50 | QA Agent | **Auth pages batch 2 (`/auth/login`, `/auth/register`, `/auth/verify`).** Exploration: `tests/exploration/findings-auth-pages-batch2.md`. Specs: `bug-163`–`bug-175` (tag `@auth-pages`; `@bug-173` in `bug-163`, `@bug-176` on `bug-165`). **REAL_BUG fixes:** `app/auth/verify/page.tsx` invalid-token state; `app/auth/register/page.tsx` duplicate-email copy; `app/auth/login/page.tsx` password-toggle `data-testid` + `aria-label`; `app/auth/register/layout.tsx` metadata title; `tests/e2e/_shared/auth.ts` login retry; mock routes use `/auth/v1/token` regex. **`npx playwright test --grep "@auth-pages" --project=chromium-desktop` → 21 passed.** Re-run: same command from `apps/med/`. Refresh setup if `bug-172` skips: `npx playwright test --project=setup`. |
| 2026-05-21 15:45 | QA Agent | **Public pages batch 1 (no-login routes).** Exploration: `tests/exploration/findings-public-pages-batch1.md`. Specs: `bug-155-public-profile-route-auth` through `bug-162-survey-hcmute-splash-i18n-bullets` (tag `@public-pages`). Fixes: `contexts/LanguageContext.tsx` sync initial lang from `localStorage`; `components/survey/SurveyLanding.tsx` bullet i18n. **`npx playwright test --grep "@public-pages" --project=chromium-desktop` → 12 passed, 1 skipped** (profile sidebar when `tests/.auth/learner.json` session expired / setup login timeout). Re-run batch: same command from `apps/med/`. |
