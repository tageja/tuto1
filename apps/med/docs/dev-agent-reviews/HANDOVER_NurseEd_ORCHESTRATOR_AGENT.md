...
| V | Feedback Submission UX — floating button + bottom-sheet form on every `/learn/*` page; reuses existing migration 047 + API + admin page (which were already built). Spec is delta-only: ~2 new components + 1 layout edit + ~15 new i18n keys. | ✅ Handover created (`HANDOVER_V_FEEDBACK_SUBMISSION_UX.md`). DB and API are ALREADY LIVE — Agent V is ~70% built; this handover covers only the missing learner-facing submission UX. |
| Z | Investor-Grade Metrics Dashboard at `/admin/metrics` (super-admin only) — 3 hero metrics (WAU/MAU + 12-week sparkline, composite rating with per-question breakdown, engagement/streak %), plus a fast-facts row. Uses already-installed `recharts`. New `GET /api/metrics/platform`, optional migration 054 for SQL views. | ✅ Handover created (`HANDOVER_Z_INVESTOR_METRICS_DASHBOARD.md`). NOT yet built. Distinct from existing `/admin/analytics` (hospital-scoped) — kept separate by design. |
...
**Agent V — Feedback Submission UX** *(handover written 2026-05-05 PM #3)*
The DB, API, admin page, and learner history page are all already live. Only the *submission* path is missing. Floating button at the bottom-right of every `/learn/*` page → bottom-sheet (mobile) or centred modal (desktop) with a 4-category picker + 10-500-char message → POST to existing `/api/feedback`. Auto-captures `pageContext` from `usePathname()`. ~3-5h work for a focused FE agent. See `HANDOVER_V_FEEDBACK_SUBMISSION_UX.md`. **Block on this before opening MVP to first learners** — without it, learners can't tell us when something breaks.

**Agent Z — Investor-Grade Metrics Dashboard** *(handover written 2026-05-05 PM #3)*
Brand new `/admin/metrics` super-admin-only page. Three hero metrics: (1) WAU/MAU + 12-week growth sparkline, (2) composite rating from `nursed_lesson_feedback` + `nursed_peer_reviews` with per-question breakdown, (3) active-streak % + avg sessions + longest streak record. Plus a fast-facts row (total learners, recordings, lessons, courses). Uses already-installed `recharts`. ~5-8h work. See `HANDOVER_Z_INVESTOR_METRICS_DASHBOARD.md`. **Tarun uses this in investor meetings, so it must be presentation-quality.** Keep `/admin/analytics` (hospital-scoped) untouched.
