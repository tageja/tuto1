...
import {
  ...
  Layers, CheckCircle, Award, Bell, RotateCcw,
} from 'lucide-react'
...
export default function CourseOverviewClient({ course, courseId }: Props) {
  const { t } = useLang()
  ...
  useEffect(() => {
    if (course?.slug && isUuid(courseId)) {
      router.replace(`/learn/courses/${course.slug}`)
    }
  }, [course, courseId, router])

  // Expand first module by default
  useEffect(() => {
    if (course?.nursed_modules?.length > 0) {
      setExpandedModules(new Set([course.nursed_modules[0].id]))
    }
  }, [course])
          ...
          <div className="space-y-3">
            ...
            {(() => {
              ...
              function getLessonStatus(lessonId: string, lessonPublished: boolean): 'completed' | 'unlocked' | 'locked' {
                if (!lessonPublished) return 'locked'
                // Check completion first — even super_admin should see their done lessons
                if (completedLessons.has(lessonId)) return 'completed'
                // super_admin bypasses sequential locking only
              ...
              return modules.map((mod, idx) => {
                ...
                const moduleHref = `/learn/courses/${course?.slug ?? courseId}/modules/${mod.slug ?? mod.id}`
                const publishedInMod = lessons.filter(l => l.published)
                const completedInMod = publishedInMod.filter(l => completedLessons.has(l.id)).length
                const modFullyDone = publishedInMod.length > 0 && completedInMod === publishedInMod.length
                return (
                  <div key={mod.id} className="card overflow-hidden">
                    <div className="flex w-full items-center justify-between gap-2 p-4 hover:bg-surface transition-colors">
                      <div className="flex items-start gap-3 min-w-0 flex-1">
                        <div className={`w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5 ${modFullyDone ? 'bg-green-100 text-success' : 'bg-primary-light text-primary'}`}>
                          {modFullyDone ? <CheckCircle size={14} /> : idx + 1}
                        </div>
                        <div className="min-w-0 flex-1">
                          <Link
                            href={moduleHref}
                            className="font-medium text-text hover:text-primary text-left block truncate"
                            onMouseEnter={() => prefetch(moduleHref)}
                          >
                          ...
                          <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                            ...
                            {completedInMod > 0 && (
                              <span className="text-xs text-success font-medium">
                                {completedInMod}/{publishedInMod.length} done
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div id={`module-panel-${mod.id}`} className="border-t border-border">
                        ...
                        {lessons.length === 0 ? (
                          ...
                          lessons.map((lesson, lIdx) => {
                            ...
                            const lessonHref = `/learn/courses/${course?.slug ?? courseId}/lessons/${lesson.slug ?? lesson.id}`
                            return (
                              <div
                                key={lesson.id}
