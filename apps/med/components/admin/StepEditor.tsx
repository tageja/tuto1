...
function FlashCardEditor({ step, siblingSteps, onSubmit, saving, onCancel }: EditorProps) {
  ...
  /** Same logic as MatchingEditor.collectLines — collect unique English dialogue lines from siblings. */
  function collectLines(): string[] {
    const lines: string[] = []
    for (const s of siblingSteps) {
      const cfg = s.config as Record<string, unknown> | null
  ...
  function handleSave() {
    ...
    // Omit mode/sprint_seconds entirely when sprint is off — keeps configs lean
    onSubmit(config)
  }

  return (
    <div className="space-y-3">
      {/* Pull & translate banner */}
      {availableScriptCount > 0 && (
        <div className="flex items-center gap-3 p-3 bg-primary/5 border border-primary/20 rounded-lg">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-primary">
...
const DEFAULT_RATINGS = ['best', 'acceptable', 'incorrect'] as const
