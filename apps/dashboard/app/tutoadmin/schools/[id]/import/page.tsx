...
const API_BASE = '/api/tutoadmin/schools';

export default function TutoAdminImportPage() {
  ...
  const handleDownloadTemplate = useCallback(async () => {
    try {
      const res = await fetch(
        `${API_BASE}/${schoolId}/import/template?entity=${entity}`,
        { credentials: 'include' }
      );
    ...
    async (selectedFile: File) => {
      ...
      try {
        ...
        const [parseRes, mappingRes] = await Promise.all([
          fetch(`${API_BASE}/${schoolId}/import/parse`, {
            method: 'POST',
            body: formData,
            credentials: 'include',
          }),
          fetch(`${API_BASE}/${schoolId}/import/mapping?entity=${entity}`, {
            credentials: 'include',
          }),
        ]);
  ...
  const handleMappingNext = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/${schoolId}/import/validate`, {
        method: 'POST',
  ...
  const handleSaveMapping = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/${schoolId}/import/mapping`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleConfirmImport = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/${schoolId}/import/execute`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ entity, mapping, rows }),
        credentials: 'include',
