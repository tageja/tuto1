...
| **Active dev branch** | **`AppleLogin+homeRedesign2`** — branched off `origin/AppleLogin+homeRedesign`. All new mobile features go here. Deploy OTA via `eas update --branch production` (JS-only changes do not require App Store submission). |
...
### April 28, 2026 — Vercel build error: wrong import depth in new API route

- **Symptom**: Deployment `dpl_4MkJvwfXRW4zdBYpQjnM7afVMJZS` (logo upload feature) failed with `Module not found: Can't resolve '../../../../../../../lib/supabase'` in `apps/dashboard/app/api/tutoadmin/schools/[id]/logo/route.ts`.
- **Cause**: Vercel builds from `apps/dashboard/` as the project root (not the monorepo root). The file is therefore 6 levels deep from the project root, but the import used 7 `../`.
- **Fix**: commit `d0f2def` — changed to 6 `../`. Deploy `dpl_6n1DAcEdq4mwDdLSN1noR4q76Tc3` went READY.
- **Lesson**: When writing API routes in `apps/dashboard/app/api/`, count path depth from `apps/dashboard/` (the Vercel project root), not from the monorepo root.
...
16. **Vercel's project root for the dashboard is `apps/dashboard/`, not the monorepo root.** Any relative import in `apps/dashboard/app/api/**` should count depth from `apps/dashboard/`. A file at `app/api/tutoadmin/schools/[id]/logo/route.ts` needs **6** `../` to reach `lib/supabase`, not 7.

17. **Mobile app active dev branch is `AppleLogin+homeRedesign2`** (not `main`). This branch is v2.2.0, 12 commits ahead of `main`. All new mobile features must be branched from here, not `main`. Deploy mobile changes via `eas update --branch production` (OTA, no App Store submission needed for JS-only changes).
...
8. **Mobile branding OTA deploy pending.** Branch `AppleLogin+homeRedesign2` has the school-branded `AppLoadingScreen` + dynamic logo in `HomeScreen` / `SchoolHeader` committed but NOT yet live. Tarun must pull this branch on Mac and run `eas update --branch production` to push it to users.

9. **Empower English onboarding next steps (as of 2026-04-28):**
   - Upload Empower English logo via `/tutoadmin/schools/[id]` (school-logos bucket is live ✅)
   - Send welcome email to Ms Nhu Le with admin code (drafted ✅)
   - Send feature walkthrough PPT (in progress)
   - Send contract draft (in progress)
   - Test sign-out flow — was not explicitly confirmed fixed; verify before customer handoff

10. **`feat/mp-a-platform-feedback` branch** — merged to `main` on 2026-04-28 (via the wrong-school-redirect fix commit `28fcfea`). Verify the PR is formally closed on GitHub if needed.
...
| 2026-04-28 | (Cursor agent) | **School branding feature (mobile + dashboard).** Two commits shipped: (1) `deaa697` on branch `AppleLogin+homeRedesign2` — mobile app: added `logo_url` to `School` type + `SchoolContext`, new `AppLoadingScreen` component (school logo, name, animated dots, "Powered by tuto." footer; falls back to Tuto dark splash if no logo), replaced `ActivityIndicator` in `RoleGate` with `AppLoadingScreen`, made `HomeScreen` header and `SchoolHeader` show school logo when `logo_url` is set (fallback to Tuto logo). (2) `8719d56` + `d0f2def` on `main` — dashboard: new API route `POST/DELETE /api/tutoadmin/schools/[id]/logo` uploads to Supabase storage bucket `school-logos` + updates `schools.logo_url`; tutoadmin school detail page shows logo preview and upload/remove UI. `logo_url` column already existed in migration `001`. Tarun manually created `school-logos` Supabase storage bucket (public). Fixed Vercel build failure caused by 1 extra `../` in import path (Lesson #16). Production deploy READY: `dpl_6n1DAcEdq4mwDdLSN1noR4q76Tc3`. **Mobile changes NOT yet OTA-deployed** — Tarun must run `eas update --branch production` from the `AppleLogin+homeRedesign2` branch on Mac to push to live app. Also confirmed: co-admins are supported via multiple admin code generation (each `is_single_use=true`). |
| | | |
